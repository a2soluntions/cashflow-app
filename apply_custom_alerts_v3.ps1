...
$stateOld = '(?s)const \[loading, setLoading\] = useState\(false\);.*?const fileInputRef = useRef<HTMLInputElement>\(null\);'
$stateNew = @'
...
$deleteOld = '(?s)const handleDeleteContent = async \(id: string\) => \{.*?confirm\(.*?\).*?\}'
$deleteNew = @'
...
# Fix 3: UI Injection (Opening)
$uiOpenOld = '(?s)return \(.*?<div className="min-h-full'
$uiOpenNew = "return (`n    <>`n      <CustomAlert />`n      <div className=`"min-h-full"
$content = $content -replace $uiOpenOld, $uiOpenNew

# Fix 4: UI Injection (Closing)
$uiCloseOld = '(?s)</div>[\s\r\n]+\);[\s\r\n]+\};'
$uiCloseNew = "</div>`n    </>`n  );`n};"
$content = $content -replace $uiCloseOld, $uiCloseNew
