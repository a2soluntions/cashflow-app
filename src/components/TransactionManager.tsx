...
export function TransactionManager() {
  ...
  return (
    <div className="h-full flex flex-col gap-6 text-white animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="flex justify-between items-center bg-white/5 backdrop-blur-md p-6 rounded-[2.5rem] border border-white/10 shadow-xl">
        <div>
        ...
        <button 
          ...
          className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-2xl font-black uppercase text-xs tracking-widest transition-all shadow-lg shadow-indigo-500/30 flex items-center gap-2 hover:scale-105"
        >
      ...
      <div className="flex gap-4 items-center">
        <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 flex items-center gap-3 focus-within:border-indigo-500/50 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar bg-white/5 border border-white/10 rounded-[2.5rem] p-4 shadow-xl">
        {filteredTransactions.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredTransactions.map(tx => {
              ...
              return (
                <div key={tx.id} className="group bg-slate-900/50 hover:bg-slate-800/80 border border-white/5 p-4 rounded-2xl flex items-center justify-between transition-all">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${isIncome ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0f172a] w-full max-w-md rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex flex-col gap-5">
              <div className="flex bg-slate-900 rounded-xl p-1 border border-white/5">
                <button 
                  ...
                  className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all flex items-center justify-center gap-2 ${formData.type === 'expense' ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' : 'text-slate-500 hover:text-rose-400'}`}
                >
                  <ArrowDownRight size={14}/> Despesa
                </button>
                <button 
                  onClick={() => setFormData({...formData, type: 'income'})} 
                  className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all flex items-center justify-center gap-2 ${formData.type === 'income' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'text-slate-500 hover:text-emerald-400'}`}
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-lg font-black text-white focus:border-indigo-500 focus:bg-indigo-500/5 outline-none transition-all"
                    placeholder="0.00"
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-sm font-bold text-white focus:border-indigo-500 focus:bg-indigo-500/5 outline-none transition-all"
              ...
              {budgetStatus && budgetStatus.hasBudget && (
                <div className={`p-4 rounded-xl border animate-in slide-in-from-top-2 flex items-start gap-4 shadow-lg transition-colors
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1">
                  ...
                  <select 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-2 py-3 mt-1 text-[11px] font-bold text-white focus:border-indigo-500 outline-none appearance-none"
                  >
                ...
                <div className="col-span-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-2 py-3 mt-1 text-[11px] font-bold text-white focus:border-indigo-500 outline-none appearance-none cursor-text"
                  />
                </div>
                ...
                <div className="col-span-1">
                  ...
                  <select 
                    ...
                    className={`w-full bg-white/5 border border-white/10 rounded-xl px-2 py-3 mt-1 text-[11px] font-bold focus:border-indigo-500 outline-none appearance-none
            ...
            <div className="p-6 border-t border-white/5 bg-black/20">
              <button 
                onClick={handleSave}
                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
              >
                Registrar Transação
              </button>
            </div>
