    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-rose-500 hover:bg-rose-600 text-white p-3 rounded-2xl transition-all shadow-lg"
        >
          <Plus size={20} />
        </button>
      </div>
      ...
      {isAdding && (
        <div className="bg-slate-50 dark:bg-white/5 p-6 rounded-[2rem] border-2 border-dashed border-rose-200 dark:border-rose-500/30 animate-in fade-in zoom-in duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <select 
              className="bg-white dark:bg-black/20 border-none rounded-xl p-3 text-xs font-bold"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {budgets.map(budget => (
          <div key={budget.id} className="bg-white dark:bg-black/20 border dark:border-white/10 p-5 rounded-[2rem] group relative">
