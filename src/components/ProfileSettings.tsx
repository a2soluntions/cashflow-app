      ...
      <div className="flex items-center justify-between pb-8 border-b border-white/5 mb-8">
          <div className="flex items-center gap-6">
              <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  <div className="w-24 h-24 rounded-2xl border border-white/10 overflow-hidden bg-black flex items-center justify-center shadow-2xl transition-all group-hover:border-[#00d06c]/50">
                      {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" alt="User" /> : <User className="w-12 h-12 text-zinc-800" />}
                  </div>
                  <div className="absolute -bottom-2 -right-2 bg-[#00d06c] p-2 rounded-lg text-black shadow-lg">
                      <Camera className="w-4 h-4" />
                  </div>
                  <input type="file" ref={fileInputRef} onChange={handleImageChange} accept="image/*" className="hidden" />
              </div>
              <div>
                  <h1 className="text-4xl font-black text-white tracking-tighter italic uppercase">{fullName || 'Comandante'}</h1>
                  <p className="text-xs text-[#00d06c] font-black uppercase tracking-[0.4em] flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#00d06c] animate-pulse" />
                    {companyName || 'VittaCash System'}
                  </p>
              </div>
          </div>
          <button onClick={onClose} className="flex items-center gap-2 px-6 py-3 rounded-full bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-widest transition-all border border-white/5">
            ...
            <div className="space-y-12">
                <section>
                    ...
                    <form onSubmit={handleSave} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-zinc-600 uppercase ml-2">Nome do Titular</label>
                            <input 
                                type="text" 
                                value={fullName} 
                        ...
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-zinc-600 uppercase ml-2">Organização</label>
                            <input 
                                type="text" 
                                value={companyName} 
                        ...
                        <button type="submit" className="w-full py-4 mt-4 rounded-2xl bg-[#00d06c] text-black font-black uppercase text-xs tracking-[0.2em] hover:brightness-110 active:scale-[0.98] transition-all shadow-xl shadow-[#00d06c]/10 flex items-center justify-center gap-2">
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={handleExportBackup} className="flex flex-col items-center gap-3 p-6 rounded-3xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-all group">
                            <Download className="w-6 h-6 text-indigo-400" />
                            <span className="text-[10px] font-black text-white uppercase tracking-widest">Exportar JSON</span>
                        </button>
                        <button onClick={() => backupInputRef.current?.click()} className="flex flex-col items-center gap-3 p-6 rounded-3xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-all">
                            <Upload className="w-6 h-6 text-emerald-400" />
                            <span className="text-[10px] font-black text-white uppercase tracking-widest">Restaurar Dados</span>
                        </button>
                ...
                <section className="pt-8 border-t border-white/5">
                    <div className="flex items-center justify-between p-6 rounded-3xl bg-white/[0.02] border border-white/5">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-xs font-black text-white uppercase tracking-widest">A2 Solutions</p>
                                <p className="text-[10px] text-zinc-500 font-bold">Suporte Direto</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <a href="https://wa.me/5534998408962" className="p-3 bg-[#00d06c] hover:brightness-110 rounded-xl transition-all text-black shadow-lg shadow-[#00d06c]/10"><MessageCircle className="w-5 h-5" /></a>
                        </div>
                    </div>
                </section>
            </div>
