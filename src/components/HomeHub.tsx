        ...
        {menuItems.map((item, index) => {
          ...
          return (
            <div key={item.id} className="absolute z-40 flex flex-col items-center group transition-all duration-700" style={{ transform: `translate(${x}px, ${y}px)` }}>
              <span className="absolute -top-12 opacity-0 group-hover:opacity-100 group-hover:-translate-y-2 transition-all duration-300 pointer-events-none whitespace-nowrap bg-black/80 backdrop-blur-md text-[10px] font-black uppercase tracking-[0.2em] text-white py-1.5 px-4 rounded-lg border border-white/10 shadow-2xl">
                {item.label}
              </span>
              <button onClick={item.action} className={`p-4 rounded-2xl ${item.color} text-white shadow-xl transition-all duration-300 hover:scale-125 hover:rotate-[360deg] active:scale-90`} style={{ boxShadow: `0 0 25px ${item.color}55` }}>
                {item.icon}
              </button>
            </div>
          );
          ...
          <div className={`w-[400px] p-8 rounded-[2.5rem] animate-in zoom-in slide-in-from-bottom-8 duration-500 shadow-2xl ${
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex gap-4 w-full">
                ...
                <button 
                  onClick={confirmExit}
                  className="flex-1 py-4 rounded-2xl bg-rose-600 text-white text-[10px] font-black uppercase tracking-[0.2em] hover:bg-rose-500 shadow-lg shadow-rose-600/20 active:scale-95 transition-all"
                >
                  Confirmar Saída
                </button>
              </div>
