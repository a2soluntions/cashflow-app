...
export default function SalesPage({ onSelectPlan }: { onSelectPlan: (plan: string) => void }) {
 ...
 const [news, setNews] = useState<News[]>([]);
  const [heroBanners, setHeroBanners] = useState<{ left?: string, right?: string }>({});
 ...
 .limit(3);
      ...
      if (bData) {
        const bMap: any = {};
        bData.forEach(b => { if (b.content_type === 'home_banner_left') bMap.left = b.image_url; if (b.content_type === 'home_banner_right') bMap.right = b.image_url; });
        setHeroBanners(bMap);
      }
 ...
 <div className="fixed top-0 left-0 w-full bg-[#050505]/90 backdrop-blur-md border-b border-white/5 py-3 overflow-hidden whitespace-nowrap z-[1000] flex justify-start">
 ...
 {/* Banner Esquerdo */}
 <div className="hidden xl:block w-48 h-80 rounded-[2.5rem] overflow-hidden border border-white/5 bg-zinc-900/50 group relative shrink-0">
 <div className="absolute top-0 left-0 w-full p-2 text-[8px] font-black uppercase tracking-widest text-zinc-600 text-center z-20">Patrocínio</div>
 <img src={heroBanners.left || "vitta_sponsor_banner_1_1776825587725.png"} alt="Sponsor" className="w-full h-full object-cover opacity-40 group-hover:opacity-100 transition-opacity duration-700" />
 <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-transparent z-10" />
 </div>

 <div className="flex-1 text-center">
 <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border-emerald-500/20 mb-8">
 ...
 className="px-10 py-5 bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase tracking-widest rounded-2xl transition-all -2xl -emerald-500/20 active:scale-95"
 >
 Assumir o Controle
 </button>
 <button 
 onClick={scrollToPlans}
 className="px-10 py-5 bg-white/5 border-white/10 hover:bg-white/10 text-white font-black uppercase tracking-widest rounded-2xl transition-all active:scale-95"
 >
 Ver Planos
 </button>
 </div>
  ...
  <div className="hidden xl:block w-48 h-80 rounded-[2.5rem] overflow-hidden border border-white/5 bg-zinc-900/50 group relative shrink-0">
 <div className="absolute top-0 left-0 w-full p-2 text-[8px] font-black uppercase tracking-widest text-zinc-600 text-center z-20">Patrocínio</div>
 <img src={heroBanners.right || "vitta_sponsor_banner_2_1776825608562.png"} alt="Sponsor" className="w-full h-full object-cover opacity-40 group-hover:opacity-100 transition-opacity duration-700" />
 <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-transparent z-10" />
 </div>
 </div>
 </section>
 ...
 {news.map(item => (
 <div key={item.id} className="group relative overflow-hidden rounded-[3rem] bg-zinc-900/50 border-white/5 hover:border-emerald-500/30 transition-all flex flex-col">
 {item.image_url && (
 ...
 {/* FREE */}
 <div className="bg-zinc-900/40 border-white/5 rounded-[4rem] p-12 flex flex-col hover:border-white/10 transition-all">
 <div className="mb-10">
 ...
 <button onClick={() => onSelectPlan('free')} className="w-full py-5 bg-white/5 border-white/10 hover:bg-white/10 rounded-3xl font-black uppercase text-[10px] tracking-widest transition-all italic">
 Experimentar Grátis
 </button>
 </div>

 {/* BÁSICO */}
 <div className="bg-zinc-900/40 border-white/5 rounded-[4rem] p-12 flex flex-col hover:lue-500/20 transition-all relative">
 <div className="mb-10">
 ...
 <button onClick={() => onSelectPlan('basic')} className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-3xl font-black uppercase text-[10px] tracking-widest transition-all -blue-500/20 italic active:scale-95">
 Assinar Agora
 </button>
 </div>

 {/* PREMIUM */}
 <div className="bg-emerald-500/5 border-2 border-emerald-500 rounded-[4rem] p-12 flex flex-col relative -[0_0_100px_rgba(16,185,129,0.15)]">
 ...
 <button onClick={() => onSelectPlan('premium')} className="w-full py-6 bg-emerald-500 hover:bg-emerald-400 text-black rounded-3xl font-black uppercase text-[10px] tracking-widest transition-all -2xl -emerald-500/20 italic active:scale-95">
 Ativar Modo PRO
 </button>
 </div>
