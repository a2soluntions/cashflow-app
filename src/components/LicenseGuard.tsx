...
const LicenseGuard: React.FC<LicenseGuardProps> = ({ onUnlock }) => {
 ...
 let id = localStorage.getItem('a2mentor_machine_id');
 if (!id) {
 id = Math.random().toString(36).substring(7);
 localStorage.setItem('a2mentor_machine_id', id);
 }
 setMachineId(id);

 // 2. Verifica se já tem chave salva
 const savedKey = localStorage.getItem('a2mentor_saved_key');
 if (savedKey) {
 ...
 // SUCESSO ABSOLUTO
 localStorage.setItem('a2mentor_saved_key', cleanKey); // Garante que salvou
 setLoading(false);
 ...
 <div className="bg-[#0f172a] w-full max-w-md p-8 rounded-[2rem] border-slate-800 -2xl relative overflow-hidden">
 ...
 className="w-full bg-[#1e293b] border-2 border-slate-800 focus:border-emerald-500 rounded-xl py-4 pl-12 pr-4 text-white font-mono text-center text-sm tracking-wider placeholder:text-slate-600 outline-none transition-all -inner uppercase"
 ...
 {error && (
 <div className="bg-rose-500/10 border-rose-500/20 rounded-xl p-3 flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
 ...
 className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-xl font-black uppercase text-xs tracking-[0.2em] -emerald-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
 >
