  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-500 p-2">
      ...
      <div className="flex justify-between items-end border-b border-slate-100 dark:border-zinc-800 pb-4">
        ...
        {/* Card Resumo Global */}
        <div className="bg-slate-900 text-white px-6 py-3 rounded-2xl shadow-lg flex items-center gap-6">
      ...
      <div className="flex gap-6 h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
          {budgetData.comparison.map((item) => (
            <div key={item.category} className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-5 shadow-sm hover:shadow-md transition-all group">
              <div className="flex justify-between items-center mb-3">
                
                {/* Nome e Ícone */}
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${item.status === 'danger' ? 'bg-rose-100 text-rose-600' : item.status === 'warning' ? 'bg-yellow-100 text-yellow-600' : 'bg-slate-100 text-slate-600'} dark:bg-black/40`}>
                ...
                <div className="flex items-center gap-3">
                  {editingCategory === item.category ? (
                    <div className="flex items-center gap-2 bg-slate-50 dark:bg-black p-1 rounded-lg border border-indigo-200 animate-in fade-in zoom-in">
                      <input 
                        type="number" 
                        autoFocus
              ...
              {/* Barra de Progresso */}
              <div className="relative h-4 w-full bg-slate-100 dark:bg-black rounded-full overflow-hidden mb-2">
                <div 
        ...
        <div className="w-1/3 flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-slate-100 dark:border-zinc-800 shadow-sm flex-1 flex flex-col">
            ...
            {/* Card de Alerta */}
            <div className={`p-6 rounded-[2rem] border ${budgetData.totalProgress > 90 ? 'bg-rose-50 border-rose-200 dark:bg-rose-900/20' : 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20'}`}>
