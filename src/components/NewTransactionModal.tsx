...
export default function NewTransactionModal({ isOpen, onClose, onSave, isLimitReached = false, theme = 'blue' }: NewTransactionModalProps) {
 ...
 useEffect(() => {
 if (isOpen) {
  const savedCats = JSON.parse(localStorage.getItem('a2mentor_pro_categories') || '[]');
 ...
 setBudgets(JSON.parse(localStorage.getItem('a2mentor_pro_budgets') || '[]'));
 setAllTransactions(JSON.parse(localStorage.getItem('a2mentor_pro_transactions') || '[]'));
 ...
 if (sendToWhatsApp) {
   ...
   if (savedPhone) {
     ...
     const messageText = 
       `*A2 Mentor - Alerta de Lançamento*\n\n` +
       `*Tipo:* ${typeLabel}\n` +
