...
import { 
  ...
  AlertCircle, Search, X, Loader2, TrendingUp,
...
export default function BillsManager({ mode = 'normal' }: BillsManagerProps) {
  ...
  useEffect(() => { setCurrentPage(0); }, [searchTerm, filterCategory, activeTab]);
  ...
  const filteredBills = bills.filter(b => {
    ...
    const matchSearch = b.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchCategory = filterCategory === 'Todas' || b.category === filterCategory;
    if (!matchType || !matchSearch || !matchCategory) return false;
    if (mode === 'overdue') {
  ...
  const categories = ['Todas', ...Array.from(new Set(
    bills.filter(b => b.type === activeTab).map(b => b.category).filter(Boolean)
  )).sort()];

  // Paginação
  const totalPages = Math.max(1, Math.ceil(filteredBills.length / ITEMS_PER_PAGE));
  const safePage = Math.min(currentPage, totalPages - 1);
  const pageBills = filteredBills.slice(safePage * ITEMS_PER_PAGE, (safePage + 1) * ITEMS_PER_PAGE);
  ...
  return (
    <div className="min-h-full w-full flex flex-col gap-4 font-sans p-2">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-12 gap-4 pb-2">
        ...
        {/* ESQUERDA: Resumo + Filtros */}
        <div className="col-span-12 md:col-span-4 flex flex-col gap-3 min-h-0 shrink-0">
          ...
          <div className="p-6 relative overflow-hidden"
            ...
            <p className="text-[10px] font-black uppercase tracking-widest mb-1 relative z-10" style={{ color: 'var(--text-muted)' }}>Total Pendente</p>
            <h1 className="text-3xl font-black tracking-tighter relative z-10" style={{ color: activeTab === 'expense' ? '#FF4757' : '#00D4AA' }}>
              {formatCurrency(totalValue)}
            </h1>
            <p className="text-[9px] font-bold mt-1 relative z-10" style={{ color: 'var(--text-muted)' }}>
              {filteredBills.length} {filteredBills.length === 1 ? 'conta pendente' : 'contas pendentes'}
            </p>
          </div>
          ...
          <div className="p-3 flex flex-col gap-2"
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-8 pr-3 py-2 text-[10px] font-black uppercase tracking-wider outline-none rounded-lg" 
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-8 pr-3 py-2 text-[10px] font-black uppercase tracking-wider outline-none rounded-lg appearance-none cursor-pointer"
                style={{ background: 'var(--bg-surface)', border: '1px solid var(--bg-border)', color: 'var(--text-primary)' }}
              >
                {categories.map(cat => (
          ...
          <div className="p-4 flex gap-3 items-start border"
            ...
            <AlertCircle size={15} style={{ color: '#6C63FF' }} className="mt-0.5 shrink-0" />
            <div>
        ...
        {/* DIREITA: Lista de Contas com paginação */}
        <div className="col-span-12 md:col-span-8 flex flex-col gap-0"
          style={{ background: 'var(--bg-card)', border: '1px solid var(--bg-border)', borderRadius: '24px', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
          ...
          <div className="flex items-center justify-between px-5 py-3 shrink-0"
            ...
            <p className="text-[9px] font-bold" style={{ color: 'var(--text-muted)' }}>
              {filteredBills.length > 0
                ? `${safePage * ITEMS_PER_PAGE + 1}–${Math.min((safePage + 1) * ITEMS_PER_PAGE, filteredBills.length)} de ${filteredBills.length}`
                : '0 itens'
              }
            </p>
          </div>
          ...
          <div className="overflow-y-auto custom-scrollbar" style={{ height: '460px' }}>
            {pageBills.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-40">
                <CheckCircle2 size={36} style={{ color: activeTab === 'expense' ? '#FF4757' : '#00D4AA' }} className="mb-3" />
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                  {filteredBills.length === 0 ? 'Tudo em dia por aqui!' : 'Nenhum resultado encontrado'}
                </p>
              </div>
            ) : (
              pageBills.map((bill) => {
                ...
                return (
                  <div key={bill.id} className="flex items-center justify-between px-5 py-3 transition-colors hover:bg-white/[0.02]"
                    style={{ borderBottom: '1px solid var(--bg-border)' }}>
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="text-xs font-black uppercase truncate" style={{ maxWidth: '260px', color: 'var(--text-primary)' }}>{bill.description}</h4>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-3 shrink-0"
              ...
              <button
                ...
                disabled={safePage === 0}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:scale-105"
                style={{ 
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: i === safePage
                        ? (activeTab === 'expense' ? '#FF4757' : '#00D4AA')
                        : 'var(--bg-border)',
                    }}
                  />
                ))}
