...
export default function DebtManager({ theme }: { theme: string }) {
  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 min-h-0">
      <div className={`shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center p-8 rounded-[3rem] border transition-all ${isLight ? 'bg-white border-slate-200 shadow-xl' : 'bg-white/5 border-white/10 backdrop-blur-xl'}`}>
        <div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-6 overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            <div className={`shrink-0 p-6 rounded-[2.5rem] border transition-all ${isLight ? 'bg-white border-slate-200 shadow-md' : 'bg-white/5 border-white/10 backdrop-blur-md'}`}>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input type="text" placeholder="EX: CARTÃO NUBANK" value={newDebt.name} onChange={e => setNewDebt({...newDebt, name: e.target.value.toUpperCase()})} className={`w-full border rounded-xl px-3 py-2.5 mt-1 text-xs font-bold outline-none transition-colors ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900 focus:border-indigo-500' : 'bg-black/20 border-white/10 text-white focus:border-indigo-500'}`} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <div className="relative mt-1">
                                ...
                                <input type="text" placeholder="0,00" value={newDebt.balanceDisplay} onChange={e => { const raw = Number(e.target.value.replace(/\D/g, "")) / 100; setNewDebt({ ...newDebt, balanceRaw: raw, balanceDisplay: raw > 0 ? raw.toLocaleString('pt-BR', {minimumFractionDigits: 2}) : '' }); }} className={`w-full border rounded-xl pl-8 pr-3 py-2.5 text-xs font-bold outline-none ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-black/20 border-white/10 text-white'}`} />
                            </div>
                        </div>
                        <div>
                            <label className="text-[9px] font-black uppercase text-slate-500 ml-1">Juros / Mês</label>
                            <div className="relative mt-1">
                                <input type="text" placeholder="0,00" value={newDebt.interestDisplay} onChange={e => setNewDebt({...newDebt, interestDisplay: e.target.value.replace(/[^0-9,]/g, '')})} className={`w-full border rounded-xl pl-3 pr-6 py-2.5 text-xs font-bold outline-none text-right ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-black/20 border-white/10 text-white'}`} />
                    ...
                    <div>
                        ...
                        <div className="relative mt-1">
                            <span className="absolute left-3 top-2.5 text-xs font-black text-slate-500">R$</span>
                            <input type="text" placeholder="0,00" value={newDebt.minPaymentDisplay} onChange={e => { const raw = Number(e.target.value.replace(/\D/g, "")) / 100; setNewDebt({ ...newDebt, minPaymentRaw: raw, minPaymentDisplay: raw > 0 ? raw.toLocaleString('pt-BR', {minimumFractionDigits: 2}) : '' }); }} className={`w-full border rounded-xl pl-8 pr-3 py-2.5 text-xs font-bold outline-none ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-black/20 border-white/10 text-white'}`} />
                        </div>
                    </div>
                    <button onClick={handleAddDebt} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all shadow-lg shadow-indigo-500/20 active:scale-95">Gravar Passivo</button>
                </div>
            </div>
            <div className={`shrink-0 p-6 rounded-[2.5rem] border transition-all flex-1 ${isLight ? 'bg-white border-slate-200 shadow-md' : 'bg-white/5 border-white/10 backdrop-blur-md'}`}>
                ...
                <div className="flex flex-col gap-2 mb-6">
                    <button onClick={() => setStrategy('avalanche')} className={`p-3 rounded-xl border flex items-center gap-3 transition-all ${strategy === 'avalanche' ? 'bg-rose-500/10 border-rose-500 text-rose-500' : 'bg-transparent border-slate-200 dark:border-white/10 text-slate-400'}`}><Zap size={16} /><span className="text-[10px] font-black uppercase tracking-widest text-left w-full">Avalanche <span className="block text-[8px] opacity-70 mt-0.5 normal-case tracking-normal">Foca no maior juro primeiro.</span></span></button>
                    <button onClick={() => setStrategy('snowball')} className={`p-3 rounded-xl border flex items-center gap-3 transition-all ${strategy === 'snowball' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-500' : 'bg-transparent border-slate-200 dark:border-white/10 text-slate-400'}`}><Snowflake size={16} /><span className="text-[10px] font-black uppercase tracking-widest text-left w-full">Bola de Neve <span className="block text-[8px] opacity-70 mt-0.5 normal-case tracking-normal">Foca na menor dívida primeiro.</span></span></button>
                </div>
                <div>
                    <label className="text-[9px] font-black uppercase tracking-widest text-indigo-500 ml-1">Aporte Mensal Extra</label>
                    <div className="flex items-center gap-3 mt-2">
                        <input type="range" min="0" max="2000" step="50" value={extraPayment} onChange={e => setExtraPayment(Number(e.target.value))} className="flex-1 accent-indigo-500 h-1.5 rounded-full bg-slate-200 dark:bg-black/30 appearance-none cursor-pointer" />
                        <span className={`text-xs font-black px-3 py-1.5 rounded-lg border w-20 text-center ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-black/30 border-white/10 text-white'}`}>{extraPayment}</span>
                    </div>
                </div>
            </div>
            {simulation?.base.isInfinite && (
                <div className="shrink-0 p-4 bg-rose-500/10 border border-rose-500/50 rounded-3xl animate-in slide-in-from-top-2 text-center">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 min-h-0 h-full">
            {simulation && !simulation.base.isInfinite && debts.length > 0 && (
                <div className="shrink-0 bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[3rem] p-6 shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center gap-6">
                    <Timer className="absolute right-[-20px] top-[-20px] size-48 text-white/10 rotate-12" />
                    <div className="text-center bg-black/20 p-5 rounded-3xl border border-white/10 shrink-0 min-w-[150px] relative z-10 text-white">
                    ...
                    <div className="flex-1 relative z-10 w-full">
                        ...
                        {extraPayment > 0 && simulation.savedTime > 0 ? (
                            <div className="flex items-center gap-6 bg-emerald-500/20 border border-emerald-500/30 p-3 rounded-2xl animate-in zoom-in-95">
            ...
            <div className={`flex-1 flex flex-col min-h-0 rounded-[3rem] p-8 border transition-all ${isLight ? 'bg-white/60 border-slate-200/50 shadow-sm' : 'bg-white/5 border-white/5 backdrop-blur-md'}`}>
                <div className="flex justify-between items-center mb-6 shrink-0">
                    ...
                    {debts.length > 0 && <span className="text-[10px] font-black uppercase tracking-widest text-rose-500 bg-rose-500/10 px-3 py-1.5 rounded-lg border border-rose-500/20">Total: R$ {totalBalance.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>}
                </div>
                {debts.length === 0 ? (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                        {debts.map(d => (
                            <div key={d.id} className={`shrink-0 p-4 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between group transition-all ${isLight ? 'bg-white border-slate-200 shadow-sm hover:shadow-md' : 'bg-black/20 border-white/10 hover:bg-white/10'}`}>
