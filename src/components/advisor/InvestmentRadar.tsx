...
export function InvestmentRadar({ theme = 'dark' }: InvestmentRadarProps) {
 ...
 const [newsFeed, setNewsFeed] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    fetchNews();
    const subscription = supabase.channel('site_content_changes').on('postgres_changes', { event: '*', schema: 'public', table: 'site_content' }, fetchNews).subscribe();
 ...
 <div className={`px-5 py-3 rounded-2xl text-center ${isLight ? 'bg-white' : 'bg-black/40'}`}>
 ...
 <div className={`hidden md:block px-5 py-3 rounded-2xl text-center ${isLight ? 'bg-white' : 'bg-black/40'}`}>
 ...
 {/* 2. RADAR DE NOTÍCIAS & ATUALIZAÇÕES */}
 {newsFeed.length > 0 && (
 <section className="mb-2">
 <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 mb-6 flex items-center gap-2">
 <Newspaper size={14}/> Radar de Mercado & Atualizações
 </h2>
 <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 flex-nowrap md:flex-wrap overflow-x-auto md:overflow-x-visible pb-4 no-scrollbar">
 {newsFeed.map((item) => (
 <div key={item.id} className={`flex flex-col p-6 rounded-[2rem] transition-all hover:scale-[1.02] shrink-0 w-80 md:w-auto ${isLight ? 'bg-white ' : 'bg-[#09090b] border-white/5'}`}>
 {item.image_url && (
 <div className="w-full h-32 rounded-xl mb-4 overflow-hidden shrink-0">
 <img src={item.image_url} alt={item.title} className="w-full h-full object-cover" />
 </div>
 )}
 <div className="flex items-center gap-2 mb-3">
 <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${item.content_type === 'news' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
 {item.content_type === 'news' ? 'Notícia' : 'Atualização'}
 </span>
 </div>
 <h3 className={`text-sm font-black uppercase tracking-wide mb-2 leading-tight ${isLight ? 'text-slate-900' : 'text-white'}`}>{item.title}</h3>
 <p className="text-xs text-slate-400 leading-relaxed flex-1 mb-4">{item.description}</p>
 {item.meta_value?.external_url && (
 <a href={item.meta_value.external_url} target="_blank" rel="noreferrer" className={`mt-auto inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-colors ${isLight ? 'text-indigo-600 hover:text-indigo-500' : 'text-indigo-400 hover:text-indigo-300'}`}>
 ...
 {/* CONSERVADOR */}
 <div className={`p-6 flex flex-col items-start rounded-[2rem] transition-all hover:scale-[1.02] ${isLight ? 'bg-white ' : 'bg-[#09090b] border-white/5'}`}>
 ...
 <div className="flex justify-between items-center bg-black/20 p-2.5 rounded-lg border-white/5">
 ...
 <div className={`flex-1 p-8 rounded-[3rem] relative overflow-hidden ${isLight ? 'bg-white ' : 'bg-[#09090b] border-white/5'}`}>
 ...
 <div className={`p-4 rounded-2xl ${isLight ? 'bg-slate-50 ' : 'bg-[#09090b] border-white/5'}`}>
 ...
 <div className="flex-1 h-8 bg-[#00d06c]/10 rounded-full overflow-hidden relative -[0_0_20px_rgba(0,208,108,0.2)]">
 ...
 <div className={`p-6 rounded-[2rem] h-full flex flex-col justify-center text-center ${isLight ? 'bg-white ' : 'bg-[#09090b] border-white/5'}`}>
