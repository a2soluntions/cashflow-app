...
const ModernSlider = ({ value, label, color }: { value: number, label: string, color: string }) => {
  ...
  return (
    <div className="flex flex-col gap-1 w-full p-2 bg-white/5 border border-white/5 rounded-xl transition-all hover:bg-white/10">
...
export const DONUT_GRADIENTS = [
  { id: 'grad0', from: '#2575fc', to: '#00f2fe', css: 'linear-gradient(to right, #2575fc, #00f2fe)' },
  { id: 'grad1', from: '#f9d423', to: '#f83600', css: 'linear-gradient(to right, #f9d423, #f83600)' },
  { id: 'grad2', from: '#fc00ff', to: '#ff0844', css: 'linear-gradient(to right, #fc00ff, #ff0844)' },
  { id: 'grad3', from: '#00dbde', to: '#fc00ff', css: 'linear-gradient(to right, #00dbde, #fc00ff)' },
  { id: 'grad4', from: '#b224ef', to: '#7579ff', css: 'linear-gradient(to right, #b224ef, #7579ff)' },
];
  ...
  return (
    <div className="relative flex items-center justify-center transition-all duration-500" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center flex-col">
        <span className="text-[9px] font-normal text-white/50 mb-0.5">{centerLabel || 'Status'}</span>
        <div className="flex items-baseline leading-none">
          <span className="text-xl font-light text-white tracking-tighter" style={{ fontSize: size * 0.25 }}>{centerValue !== undefined ? centerValue : 100}</span>
          <span className="text-[9px] font-light text-white/50 ml-0.5">%</span>
        </div>
      </div>
    </div>
  );
...
export default function DashboardHome({ transactions, categories = [] }: DashboardProps) {
 ...
 />
  ...
  <h1 className="text-xl font-black uppercase tracking-tighter flex items-center gap-2 italic text-white">
  <Activity size={20} className="text-emerald-500" /> Cockpit
  </h1>
  <div className="flex bg-[#09090b] p-0.5 rounded-lg border border-white/5 items-center">
  {viewMode === 'monthly' && (
    <>
      <select 
        value={selectedMonth} 
      ...
      >
        {['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'].map((m, i) => (
          <option key={i} value={i + 1} className="bg-[#09090b] text-white">{m}</option>
        ))}
      </select>
      <div className="w-[1px] h-4 bg-white/10 mx-0.5"></div>
    </>
  )}
  <button onClick={() => setViewMode('monthly')} className={`px-5 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all rounded-md ${viewMode === 'monthly' ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20' : 'text-slate-500 hover:text-white'}`}>Mensal</button>
  <button onClick={() => setViewMode('yearly')} className={`px-5 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all rounded-md ${viewMode === 'yearly' ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20' : 'text-slate-500 hover:text-white'}`}>Anual</button>
  </div>
  </div>

  {/* METAS HORIZONTAIS */}
  <div className="flex items-center gap-2 overflow-x-auto no-scrollbar shrink-0 mb-1 bg-white/5 p-1 rounded-xl border border-white/5">
  ...
  <div key={i} className={`px-4 py-2 rounded-lg flex items-center gap-4 shrink-0 transition-all border border-white/5 relative overflow-hidden ${budget.isOver ? 'bg-rose-500/10' : 'bg-emerald-500/10'}`}>
  ...
  <span className="text-[10px] font-black uppercase text-white truncate max-w-[90px] z-10">{budget.category}</span>
  ...
  <div className="grid grid-cols-12 gap-2 mb-1 shrink-0">
    {/* DISTRIBUIÇÃO */}
    <div className="col-span-12 lg:col-span-3 p-3 bg-white/5 border border-white/5 rounded-xl flex flex-col min-h-[220px]">
      <div className="flex-1 flex flex-col items-center justify-center p-1">
        <div className="flex items-center gap-1.5 text-blue-400 mb-2"><Percent size={16} /><span className="text-[10px] font-black uppercase tracking-widest">Distribuição</span></div>
        <DonutChart data={financialData.categoryImpact} color="#4facfe" size={130} />
      </div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 p-2 border-t border-white/5 mt-1 shrink-0 overflow-y-auto no-scrollbar max-h-[60px]">
        {financialData.categoryImpact.map((item, i) => {
          ...
          return (
            <div key={i} className="flex justify-between items-center text-[9px] font-black uppercase">
              ...
              <span className="text-white opacity-40 ml-1">{item.percent.toFixed(0)}%</span>
            </div>
          );
        })}
      </div>
    ...
    <div className="col-span-12 lg:col-span-6 flex flex-col gap-2">
      <div className="flex justify-around items-center flex-1 bg-white/5 border border-white/5 rounded-xl p-2 min-h-[220px] overflow-x-auto no-scrollbar">
        {[
          { value: financialData.healthScore, label: 'Saúde', color: '#00f2fe', icon: Activity },
          { value: financialData.expenseRatio, label: 'Uso Renda', color: '#f9d423', icon: Wallet },
          { value: financialData.debtRatio, label: 'Dívida', color: '#ff0844', icon: Skull },
          { value: financialData.savingsRate, label: 'Retenção', color: '#b224ef', icon: Target },
        ].map((metric, i) => (
    ...
    <div className="col-span-12 lg:col-span-3 p-3 bg-white/5 border border-white/5 rounded-xl flex flex-col min-h-[220px]">
      <div className="flex-1 flex flex-col items-center justify-center p-1">
        <div className="flex items-center gap-1.5 text-rose-500 mb-2"><Skull size={16} /><span className="text-[10px] font-black uppercase tracking-widest">Vilões</span></div>
        <DonutChart data={financialData.villains} color="#ff0844" size={130} />
      </div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 p-2 border-t border-white/5 mt-1 shrink-0 overflow-y-auto no-scrollbar max-h-[60px]">
        {financialData.villains.map((v, i) => {
          ...
          return (
            <div key={i} className="flex justify-between items-center text-[9px] font-black uppercase">
              ...
              <span className="text-white opacity-40 ml-1">{v.amount > 999 ? (v.amount/1000).toFixed(1)+'k' : v.amount.toFixed(0)}</span>
            </div>
          );
        })}
      </div>
  ...
  <div className="grid grid-cols-12 gap-2 mb-1 shrink-0">
    {/* FLUXO (50%) */}
    <div className="col-span-12 lg:col-span-6 p-3 bg-white/5 border border-white/5 rounded-xl flex flex-col relative min-h-[160px]">
      <div className="flex justify-between items-center mb-1.5 text-[8px] font-black uppercase tracking-widest text-slate-400 shrink-0 z-10">
        <span className="flex items-center gap-1.5"><Activity size={14} className="text-[#00f2fe]" /> {viewMode === 'monthly' ? 'Fluxo Diário' : 'Fluxo Mensal'}</span>
        <div className="flex gap-3 text-[7px]"><div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full border border-[#00f2fe] bg-transparent"/> In</div><div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full border border-[#ff0844] bg-transparent"/> Out</div></div>
      </div>
        ...
        <svg viewBox="0 0 100 30" className="w-full flex-1 overflow-visible" preserveAspectRatio="none">
          <defs>
            ...
            <linearGradient id="gradient-out" x1="0" y1="0" x2="0" y2="1">
              ...
              <stop offset="100%" stopColor="#fc00ff" stopOpacity="0.1" />
            </linearGradient>
          </defs>

          {/* AXES */}
          <polyline points="0,0 0,30 100,30" fill="none" stroke="#00f2fe" strokeWidth="0.4" opacity="0.8" />
          ...
          <path d={getLinePath(financialData.chartData, 'in')} fill="none" stroke="#00f2fe" strokeWidth="0.4" strokeLinejoin="round" />
          <path d={getLinePath(financialData.chartData, 'out')} fill="none" stroke="#ff0844" strokeWidth="0.4" strokeLinejoin="round" />
        </svg>
        
        {/* LABELS DO EIXO X */}
        <div className="flex justify-between w-full text-[6px] font-bold text-white/30 uppercase mt-1.5 px-0.5">
    ...
    {/* STATS (50%) */}
    <div className="col-span-12 lg:col-span-6 grid grid-cols-3 gap-2 bg-white/5 border border-white/5 rounded-xl p-3 min-h-[160px]">
      {[
        ...
        { label: 'Entradas', val: financialData.income, icon: TrendingUp, color: 'text-[#00f2fe]' },
        { label: 'Saídas', val: financialData.expense, icon: TrendingDown, color: 'text-[#ff0844]' },
        { label: 'Retenção', val: (financialData.savingsRate).toFixed(1)+'%', icon: Percent, color: 'text-[#b224ef]' },
        { label: 'Juros', val: 0, icon: Flame, color: 'text-[#f9d423]' },
      ].map((card, i) => (
        <div key={i} className="flex flex-col justify-center items-center bg-white/5 rounded-lg border border-white/5 p-2 transition-all hover:bg-white/10">
  ...
  <div className="grid grid-cols-1 md:grid-cols-12 gap-2 shrink-0">
    {/* SAÚDE */}
    <div className={`md:col-span-4 p-3 bg-white/5 border ${financialData.healthStatus === 'good' ? 'border-emerald-500/20' : financialData.healthStatus === 'danger' ? 'border-rose-500/20' : 'border-white/5'} rounded-xl flex flex-col justify-between min-h-[90px]`}>
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
        {financialData.proximosPassos.map((step, i) => (
          <div key={i} className={`px-2 py-1 bg-white/5 border ${financialData.healthStatus === 'good' ? 'border-emerald-500/20' : financialData.healthStatus === 'danger' ? 'border-rose-500/20' : 'border-white/5'} rounded whitespace-nowrap`}>
    ...
    {/* VENCIMENTOS */}
    <div className="md:col-span-4 p-3 bg-white/5 border border-white/5 rounded-xl flex flex-col min-h-[90px]">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-0.5">
        {financialData.upcomingBills.length === 0 ? (
          ...
          <div key={i} className="p-2 bg-white/5 border border-white/5 rounded-lg flex flex-col min-w-[110px] shrink-0">
            <p className="text-[9px] font-black uppercase truncate text-white mb-0.5">{bill.description}</p>
    ...
    {/* AI INSIGHT */}
    <div className={`md:col-span-4 p-3 bg-gradient-to-br ${financialData.healthStatus === 'good' ? 'from-emerald-500/10 to-blue-500/10 border-emerald-500/20' : financialData.healthStatus === 'danger' ? 'from-rose-500/10 to-orange-500/10 border-rose-500/20' : 'from-white/5 to-white/5 border-white/5'} border rounded-xl flex flex-col relative overflow-hidden min-h-[90px]`}>
