...
export function FinanceQuiz() {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
          <button onClick={() => setMode('easy')} className="group p-8 bg-white/5 border border-white/10 rounded-[2.5rem] hover:border-emerald-500/50 transition-all">
          ...
          <button onClick={() => setMode('hard')} className="group p-8 bg-white/5 border border-white/10 rounded-[2.5rem] hover:border-rose-500/50 transition-all">
  ...
  if (showPhrase) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-500">
        <div className="bg-indigo-600/20 border border-indigo-500/30 p-12 rounded-[3rem] max-w-lg shadow-2xl">
  ...
  if (isFinished) {
    return (
      <div className="h-full flex flex-col items-center justify-center animate-in zoom-in duration-500">
         ...
         <button onClick={() => { setMode(null); setPhase(1); setIsFinished(false); setScore(0); }} className="mt-8 px-10 py-4 border border-white/20 rounded-full text-white font-black uppercase hover:bg-white/10 transition-all">
           Reiniciar Jornada
         </button>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-in fade-in duration-500">
      {/* HUD DE FASE */}
      <div className="flex justify-between items-center px-4 bg-white/5 p-4 rounded-[2rem] border border-white/10">
        ...
        <div className="flex gap-1">
          {[1,2,3,4,5].map(i => (
            <div key={i} className={`h-1.5 w-8 rounded-full ${i <= currentInPhase + 1 ? 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'bg-white/10'}`} />
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-3xl mx-auto w-full">
        <div className="w-full bg-white/5 backdrop-blur-2xl border border-white/10 p-10 rounded-[3rem] relative overflow-hidden shadow-2xl">
          ...
          <div className="grid gap-3">
            {q.options.map((opt, idx) => {
              ...
              if (showResult) {
                if (mode === 'easy') {
                  if (idx === q.correct) btnStyle = "bg-emerald-500/20 border-emerald-500/50 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]";
              ...
              return (
                <button
                  ...
                  className={`p-5 rounded-2xl border text-left font-bold text-sm transition-all duration-300 ${btnStyle}`}
                >
                  {opt}
                </button>
              );
          ...
          {showResult && mode === 'easy' && (
            <div className="mt-6 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl animate-in slide-in-from-top-2">
        ...
        {showResult && (
          <button 
            onClick={handleNext} 
            className="mt-8 group bg-white text-indigo-950 font-black uppercase px-12 py-5 rounded-full flex items-center gap-3 transition-all active:scale-95 shadow-xl"
          >
