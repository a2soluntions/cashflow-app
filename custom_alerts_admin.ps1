...
'@
$content = $content -replace 'const \[uploading, setUploading\] = useState\(false\);', "const [uploading, setUploading] = useState(false);`n$alertState"

# Fix 2: Replace native confirm in handleDeleteContent
$oldDelete = 'const handleDeleteContent = async (id: string) => {`n    if (confirm("Excluir este item?")) {`n      await supabase.from(''site_content'').delete().eq(''id'', id);`n      fetchData();`n    }`n  };'
$newDelete = @'
...
'@
$content = $content -replace 'const handleDeleteContent = async \(id: string\) => \{[\s\S]*?fetchData\(\);[\s\S]*?\}[\s\S]*?\};', $newDelete

# Fix 3: Replace native alerts in handleFileUpload and handleAddNews
$content = $content -replace 'alert\(''Erro no upload: '' \+ error\.message\);', "showAlert('Erro de Upload', error.message, 'error');"
$content = $content -replace 'alert\(''ERRO: O Bucket[\s\S]*?''\);', "showAlert('Bucket não encontrado', 'O Bucket \"vitta-assets\" não foi encontrado. Por favor, crie-o no Supabase Storage.', 'error');"
$content = $content -replace 'alert\("Erro ao salvar: " \+ err\.message\);', "showAlert('Erro ao Salvar', err.message, 'error');"

# Fix 4: Render the CustomAlert component
$content = $content -replace '<div className="min-h-full', '<CustomAlert />`n      <div className="min-h-full'

Set-Content $file $content -Encoding UTF8
Write-Host 'AdminDashboard custom alerts implemented!'
